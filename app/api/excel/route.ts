...
import fs from 'fs';
import os from 'os';
import { ExtractedResumeData } from '../../action/dataExtractor';

/**
 * 履歴書データをExcelテンプレートに転記するAPIルート
 */
export async function POST(request: NextRequest) {
  try {
    const resumeData: ExtractedResumeData = await request.json();
    ...
    if (!worksheet) {
      return NextResponse.json({ error: 'テンプレートにSheet1が見つかりません' }, { status: 500 });
    }
